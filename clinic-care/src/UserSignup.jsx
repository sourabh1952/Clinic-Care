...
const UserSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/usersignup', {
        username,
        email,
        password,
      });
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 bg-white shadow-lg rounded-lg">
        ...
        <div className="mt-4 text-center">
          <p>Already have an account? <Link to="/user-login" className="text-blue-500 hover:underline">Login here</Link></p>
        </div>
      </div>
    </div>
  );
